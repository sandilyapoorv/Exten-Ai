...
const steps = [
  ...
  },
  {
    ...
    body: 'Keep the key copied. The next step stores it locally in Firefox.',
    link: false
  },
  {
...
async function init() {
  const stored = await extApi.storage.local.get(Object.values(storageKeys));
...
els.grantAccess.addEventListener('click', async () => {
  ...
  const granted = await extApi.permissions.request({
    origins: ['http://*/*', 'https://*/*']
  });

  if (!granted) {
    els.setupStatus.textContent = 'Firefox access was not granted. Exten AI needs this to use page context.';
    return;
  }
...
els.saveKey.addEventListener('click', async () => {
  ...
  apiKey = key;
  await extApi.storage.local.set({ [storageKeys.apiKey]: apiKey });
  await showChat();
});
...
els.newChat.addEventListener('click', async () => {
  ...
  if (!isBlankThread(thread)) {
    ...
    thread = createThread([], tab && !isRestrictedTabUrl(tab.url) ? tab : null);
...
extApi.tabs?.onActivated?.addListener(async () => {
  await refreshActiveTabState();
});
...
async function showChat() {
  ...
  renderMode();
  await refreshActiveTabState({ startConsole: true });
}

async function refreshActiveTabState({ startConsole = false } = {}) {
  ...
  await syncThreadToActiveTab(tab);
  renderMessages();
  renderAttachment();
  renderHistoryList();
  renderArchiveList();
...
function setBlockedPage(blocked, message = 'Firefox does not allow extensions to read this page.') {
...
async function setMode(nextMode) {
  mode = nextMode;
  await extApi.storage.local.set({ [storageKeys.mode]: mode });
  renderMode();
}

async function hasSiteAccess() {
  return extApi.permissions.contains({
    origins: ['http://*/*', 'https://*/*']
  });
}
  ...
  await withBusy(els.attachmentTrigger, async () => {
    if (kind === 'console') {
      ...
      setStatus(pendingConsole.length
        ...
        : response.error
          ? `Console capture could not start: ${response.error}`
        : response.ready
...
async function syncThreadToActiveTab(activeTab = null) {
  const tab = activeTab || (await sendBackground({ type: 'GET_ACTIVE_TAB' }).catch(() => null))?.tab;
  if (!tab?.url || isRestrictedTabUrl(tab.url)) return;
  currentTabId = tab.id || null;
...
async function sendBackground(message) {
  const response = await extApi.runtime.sendMessage(message);
